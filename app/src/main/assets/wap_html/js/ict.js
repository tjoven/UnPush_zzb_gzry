...
function bindTable(k,v,dir,removeStyle){
	...
	if(heightcz>0){//下方补充空白区域，不然不能刷新
		if($("#"+k).children().length>1){//记录不能为空
			...
			if(!sys_klbts[k]){
				$("#"+k).append('<li><div style="font-size: 16px;background: white;height: 80px;display: -webkit-box;-webkit-box-pack: center;-webkit-box-align: center">没有数据！</div></li>');
			}
...
//人员选择
function sys_ryxz(sys_fsfw,sys_fsfw_mc,sys_bm,sys_qxid,sys_mkdm,org_bmid){
	...
	if(org_bmid===undefined){
		org_bmid = '0';
	}
	var yhfw = $("#"+sys_fsfw).val();
	var url='/org/ryxz.html?sys_fsfw='+sys_fsfw+'&sys_fsfw_mc='+sys_fsfw_mc+'&sys_bm='+sys_bm+'&yhfw='+yhfw+'&sys_qxid='+sys_qxid+'&sys_mkdm='+sys_mkdm+'&org_bmid='+org_bmid;
	if(isPad){
