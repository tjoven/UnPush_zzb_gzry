...
　　},
	...
	　　　　　　　　		sReg = this.executeFunc(checkArr[i]);
					...
					sys_alert(e+"使用错误:类型'"+checkArr[i]+"'对应的验证规则函数未定义！");
					return false;
					//sReg = ele.check;
				}
			}
...
//////////////////////////////////验证规则定义////////////////////////////////
	...
	setNumber:function (min,max){
		...
		if(min=="#"){
			if(this.value>max){
				sys_alert(this.showName+"不能大于"+max);	
				return false;
			}
		}
		if(max=="#"){
			if(this.value<min){
				sys_alert(this.showName+"不能小于"+min);	
				return false;
			}
		}
		if(this.value<min||this.value>max){
			if(min==max){
				sys_alert(this.showName+"的值应为"+min);	
			}else{
				sys_alert(this.showName+"应介于"+min+"和"+max+"之间！");	
			}
			return false;
		}
		return true;
