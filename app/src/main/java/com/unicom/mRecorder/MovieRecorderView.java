...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
	...
	private File tempVecordFile = null;// 文件
	...
	public MovieRecorderView(Context context, AttributeSet attrs, int defStyle) {
		...
		mHeight = a.getInteger(R.styleable.MovieRecorderView_height, 480);// 默认240
	...
	private void createRecordDir() {
		File sampleDir = new File(Environment.getExternalStorageDirectory() + File.separator + "Undownload/");
		if (!sampleDir.exists()) {
			sampleDir.mkdirs();
		}
		...
		// 创建文件
		try {
			tempVecordFile = File.createTempFile("recording", ".mp4", vecordDir);//mp4格式
			//若果成功  
            if(tempVecordFile.exists())  
            {  
            	int idx = tempVecordFile.toString().lastIndexOf('/');
				//int idxn = fileName.lastIndexOf('/');
            	String filename="/recording-"+TimeUtil.getCurrentDateMis2()+".mp4";
				mVecordFile=new File(tempVecordFile.toString().substring(0, idx)+filename);
                //改名  
				tempVecordFile.renameTo(mVecordFile);
            } 
			Log.i("===",mVecordFile.getAbsolutePath());
		} catch (IOException e) {
		}
	}
